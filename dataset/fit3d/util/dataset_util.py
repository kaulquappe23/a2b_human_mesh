"""
Code copied from https://github.com/sminchisescu-research/imar_vision_datasets_tools
...
def read_data(data_root, dataset_name, subset, subj_name, action_name, camera_name, subject="w_markers", needed_vals=None):
    if needed_vals is None:
        needed_vals = ['frames', 'j3ds', 'cam_params', 'gpps', 'smplx_params', 'annotations']
    frames, j3ds, cam_params, gpps, smplx_params, annotations = None, None, None, None, None, None
    ...
    if dataset_name == 'chi3d': # 2 people in each frame
        raise RuntimeError("not implemented for needed vals")
...
def project_3d_to_2d_batched(points3d, intrinsics, intrinsics_type):
    if intrinsics_type == 'w_distortion':
        p = intrinsics['p'][:, [1, 0]]
        x = points3d[:, :, :2] / points3d[:, :, 2:3]
        r2 = np.sum(x**2, axis=2)
        radial = 1 + np.transpose(np.matmul(intrinsics['k'], np.array([r2, r2**2, r2**3]).transpose(1, 0, 2)))
        radial = radial.transpose(2, 0, 1)
        tan = np.matmul(x, np.transpose(p))
        xx = x*(tan + radial) + r2[:, :, np.newaxis] * p
        proj = intrinsics['f'] * xx + intrinsics['c']
    elif intrinsics_type == 'wo_distortion':
    ...
    if points_2d.shape[0]:
        ...
        if with_ids:
            for i in range(num_points):
                ax.text(points_2d[i, 0], points_2d[i, 1], str(i), color='red', fontsize=12)
        if with_limbs:
