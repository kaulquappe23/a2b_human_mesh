...
class H36MOrder17P:
    ...
    def to_aspset_order(cls):
        return [cls.r_ankle, cls.r_knee, cls.r_hip,
                ...
                cls.head_top, cls.head,
                cls.neck, cls.torso,
                cls.pelvis,
                ]

    @classmethod
    def from_16p_without_head_top_order(cls):
        return [i for i in range(cls.head + 1)] + [cls.head] + [i for i in range(cls.head + 1, 16)]

    @classmethod
    def to_16p_order(cls):
        """
        Without head top
        :return:
        """
...
class H36MOrder16P:
    """
    Helper for the Human3.6M pose definition with points from posetriplet (only 16 points)
    """
    ...
    @classmethod
    def to_ik_order(cls):
        return [cls.r_ankle, cls.r_knee,
                cls.l_knee, cls.l_ankle,
                cls.pelvis,
                cls.neck,
                #cls.head,
                cls.r_wrist, cls.r_elbow, cls.r_shoulder,
...
class PoseTripletJointOrder:
    ...
    @classmethod
    def to_our_17_joint_order(cls, n_joints=16):
        if n_joints == 16:
            warnings.warn(f"Pose Triplet has only 16 joints. Head top with number {H36MOrder17P.head_top} for H36MOrder17P is missing.")
            return [cls.RightFoot, cls.RightLeg, cls.RightUpLeg,
                    cls.LeftUpLeg, cls.LeftLeg, cls.LeftFoot,
                    cls.Hips,
                    cls.Neck, cls.Spine2,
                    cls.Head,  # HeadTop is not available
                    cls.RightHand, cls.RightForeArm, cls.RightArm,
                    cls.LeftArm, cls.LeftForeArm, cls.LeftHand,
                    ]
        else:
            warnings.warn(
                f"Pose Triplet has only 16 joints. Head top with number {H36MOrder17P.head_top} for H36MOrder17P is missing and replaced by doubling the head keypoint.")
            return [cls.RightFoot, cls.RightLeg, cls.RightUpLeg,
                    cls.LeftUpLeg, cls.LeftLeg, cls.LeftFoot,
                    cls.Hips,
                    cls.Neck, cls.Spine2,
                    cls.Head, cls.Head,  # HeadTop is not available
                    cls.RightHand, cls.RightForeArm, cls.RightArm,
                    cls.LeftArm, cls.LeftForeArm, cls.LeftHand,
                    ]
