...
import uplift_upsample.model.vision_transformer as vit
...
class StridedTransformerBlock(nn.Module):
    ...
    def forward(self, x, pos_encoding=None, mask=None):
        ...
        y, attn = self.attn(y, attn_mask=mask)
...
class UpliftUpsampleTransformer(nn.Module):
    ...
    def __init__(
            self, in_feature_size=2, full_output=True, num_frames=9, num_keypoints=17,
            spatial_d_model=16, temporal_d_model=256,
            spatial_depth=3, temporal_depth=3,
            strides=[3, 3, 3],
            paddings=None,
            num_heads=8, mlp_ratio=2., qkv_bias=True,
            attn_drop_rate=0.1, drop_rate=0.1, drop_path_rate=(0, 0, 0), norm_layer=nn.LayerNorm,
            output_bn=False,
        ...
        :param in_feature_size: #channels of inputs, usually 2 for x,y coordinates of the 2d poses
        :param full_output: If true, the full sequence is predicted before the strided transformer is applied, and a loss is also calculated for the full sequence
        :param num_frames: Number of frames in the input sequence, The sequence length of the input is (num_frames -1) * config.MASK_STRIDE + 1
        :param num_keypoints: number of keypoints for the pose
        :param spatial_d_model: embedding dimension for the spatial transformer (within pose attention)
        :param temporal_d_model: embedding dimenstion for the temporal transformer (across pose attention)
        :param spatial_depth: number of transformer layers for the spatial transformer
        :param temporal_depth: number of transformer layers for the temporal transformer
        :param strides: stride values for the strided transformer. Is a list of stride values, one for each transformer layer
        :param paddings: paddings for the strided transformer layers, len(paddings) must be equal to len(strides)
        :param num_heads: number of heads for all transformer blocks
        :param mlp_ratio: the MLP in each transformer block after the attention consists of two layers, the hidden layer has embed_dim * mlp_ratio neurons
        :param qkv_bias: use a bias for the qkv projection or not
        :param attn_drop_rate: dropout rate used after the attention and after each fully connected layer in the MLP of the transformer blocks
        :param drop_rate: dropout rate used after the qkv projection and after each fully connected layer in the MLP of the transformer blocks
        :param drop_path_rate: Path drop rate for the transformers. The first value is for the spatial transformer, the second for the temporal transformer,
        the third for the strided transformer. Drop path means that randomly, a block(=path) of the network is disabled. The drop path rate is increased linearly from 0 to the given value for each transformer type.
        :param norm_layer: normalization layer applied in transformer blocks
        :param output_bn: use batch normalization before the final output head layers
        :param has_strided_input: indicates if the input is a sequence with a 2d pose for every frame or a strided sequence
        :param first_strided_token_attention_layer: first layer of the temporal transformer where attention is allowed from strided tokens to other tokens
        :param token_mask_rate: Additional random masking of tokens with the given rate before the temporal transformer begins
        :param learnable_masked_token: in case of token masking, mask with a learnable layer if this is true, else with zero
        :param return_attention: return the attention values additionally to the output
        """
        ...
        if len(self.strides) > 0:
            ...
            for i, s in enumerate(self.strides):
                p = 1 if paddings is None else paddings[i]
...
def build_uplift_upsample_transformer(config: UpliftUpsampleConfig):
    ...
    model = UpliftUpsampleTransformer(in_feature_size=config.IN_FEATURE_SIZE,
