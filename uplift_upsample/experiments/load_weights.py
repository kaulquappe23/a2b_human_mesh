...
if __name__ == '__main__':
    model_name = "h36m_351"
    with open(f"/data/ssd0/ludwikat/pretrained_weights/uplift-upsample/{model_name}.pkl", "rb") as f:
        weights = pickle.load(f)

    model = build_uplift_upsample_transformer(UpliftUpsampleConfig(config_file=f"{model_name}.json"))
    st_dict = model.state_dict()
    torch_weights = convert_tf_to_torch_weights(weights)
    model.load_state_dict(torch_weights, strict=True)
    torch.save(model.state_dict(), f"/data/ssd0/ludwikat/pretrained_weights/uplift-upsample/{model_name}.pth")
