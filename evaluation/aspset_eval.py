...
def flatten_aspset_dict(aspset_dict, per_subject=False):
    if per_subject:
        np_res = {}
        identifiers = {}
    else:
        np_res = []
        identifiers = []
    for s in aspset_dict:
        if per_subject:
            np_res[s] = []
            identifiers[s] = []
        for c in aspset_dict[s]:
            for cam in aspset_dict[s][c]:
                a = aspset_dict[s][c][cam]
                ids = [f"{s}_{c}_{cam}_{i}" for i in range(a.shape[0])]
                if per_subject:
                    np_res[s].append(a)
                    identifiers[s].extend(ids)
                else:
                    np_res.append(a)
                    identifiers.extend(ids)
    if per_subject:
        for s in np_res:
            np_res[s] = np.concatenate(np_res[s])
    else:
        np_res = np.concatenate(np_res)
    return np_res, identifiers
...
def get_anthro_beta_params(filename, beta_param_name):
    """
    Read anthropometric measurements from file and return the ones with a specific name as torch tensor
    :param filename: path to read
    :param beta_param_name: specific name
    :return:
    """
    with open(filename, "rb") as f:
        all_params = pickle.load(f)
    beta_params = all_params[beta_param_name]
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
    for k in beta_params:
        if k not in ["name", "gender", "num_betas"]:
            beta_params[k] = torch.from_numpy(np.asarray(beta_params[k])).to(device).float()
    return beta_params


def generate_and_evaluate(smplx_path, output_path, aspset_path, predefined_betas=None, model_name=None, force_recomputation=True, rotate_smplx_mesh=False, split="test"):
    """
    ...
    :param force_recomputation: if false, the 3D joints are loaded from disk if they exist, otherwise they are always computed
    ...
    joints_dict = evaluate_mesh_estimation_model(res,
                                                 ...
                                                 force_recalc=force_recomputation,
...
def evaluate_results_with_all_predefined_betas(res_file, output_path, aspset_path, predefined_betas_file=None, run_name="", rotate_smplx_mesh=False, force_recalc=True):
    ...
    generate_and_evaluate(smplx_path=res_file,
                          ...
                          force_recomputation=force_recalc)
